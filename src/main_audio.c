...
int audio_setup(t_audio_info* audio_info)
{
	...
	log_info("Leyendo de a %d samples a %d Hz (%.2f ms)",
		audio_info->config.fft_samples,
		audio_info->config.min_sample_rate,
		audio_info->config.fft_duration_ms
	);
	...
	audio_info->chunk.samples = (int16_t*)malloc(sizeof(int16_t) * audio_info->config.fft_samples);

	size_t fft_mem_size = sizeof(double) * audio_info->config.fft_samples;
...
{
	if (audio_info->chunk.length >= audio_info->config.fft_samples) {
	...
	read_audio(audio_info->audio_in.stream, &audio_info->chunk, audio_info->config.fft_samples);
	...
	// log_debug("Calculo FFT (%d samples = %.2f ms)", audio_info->config.fft_samples, audio_info->config.fft_duration_ms);

	// Copio la señal a dos nuevos arrays que van a necesitar las funciones de FFT
	for (int i = 0; i < audio_info->config.fft_samples; i++) {
	...
	audio_info->fft.length = audio_info->config.fft_samples;
